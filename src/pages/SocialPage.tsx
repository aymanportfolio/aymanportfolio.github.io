...
export const SocialPage: React.FC<SocialPageProps> = ({ onNavigate, theme }) => {
  ...
  const handleSocialClick = (url: string, name: string) => {
    ...
    }, `Authentication required to unlock official ${name} link.`);
  };
      ...
      >
        ...
        <div className="flex flex-col items-center text-center space-y-3 relative z-10">
          <div className="relative group">
            ...
            {/* Avatar Circle */}
            <div className="relative w-28 h-28 rounded-full overflow-hidden border-2 border-pink-400/80 bg-slate-900 flex items-center justify-center shadow-xl">
              <img
          ...
          {socialLinks.map((item) => (
            <div key={item.id} className="relative group">
              <button
                ...
                className={`w-full py-3.5 px-4 rounded-2xl border bg-gradient-to-r ${item.color} ${item.hoverGlow} backdrop-blur-xl flex items-center justify-between transition-all duration-300 hover:-translate-y-0.5 group-hover:scale-[1.02] cursor-pointer text-left`}
              >
        ...
        <div className="pt-4 border-t border-slate-800/80 text-center font-mono text-[10px] text-slate-400 space-y-1">
          <p>© 2025-2026 Ayman Ullah Portfolio • All Rights Reserved</p>
          <button
