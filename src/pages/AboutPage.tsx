...
export const AboutPage: React.FC<AboutPageProps> = ({ onNavigate, theme }) => {
  ...
  const skillCategories = [
    ...
    { name: 'Forestry & GIS', icon: <Sparkles className="w-4 h-4 text-emerald-400" /> },
    { name: 'Soil & Watershed', icon: <Building2 className="w-4 h-4 text-amber-400" /> },
    { name: 'Forest Ecology', icon: <Zap className="w-4 h-4 text-green-400" /> },
    { name: 'Wildlife & Conservation', icon: <ShieldCheck className="w-4 h-4 text-teal-400" /> },
  ...
  const filteredCertificates = CERTIFICATES_DATA.filter((cert) => {
    ...
    const matchesSkill = cert.skills.some((skill) => {
      ...
      if (filter.includes('forestry')) return s.includes('forestry') || s.includes('gis') || s.includes('remote sensing') || s.includes('climate') || s.includes('environmental');
      if (filter.includes('soil')) return s.includes('soil') || s.includes('watershed') || s.includes('geology') || s.includes('conservation');
      if (filter.includes('ecology')) return s.includes('ecology') || s.includes('silviculture') || s.includes('mangrove') || s.includes('plantation');
      if (filter.includes('wildlife')) return s.includes('wildlife') || s.includes('biodiversity') || s.includes('conservation') || s.includes('eco-tourism');
    ...
    <div className="space-y-12 pb-20 text-slate-100">
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12 relative z-10">
          ...
          <div className="relative flex-shrink-0 group">
            ...
            <div className="relative w-44 h-44 sm:w-52 sm:h-52 rounded-full overflow-hidden border-4 border-pink-500/60 bg-slate-900 flex items-center justify-center shadow-2xl">
          ...
          <div className="space-y-4 text-center lg:text-left flex-1">
            ...
            <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-sans max-w-2xl">
              Electrical & Electronic Engineering graduate and active <strong className="text-cyan-300 font-semibold">B.Sc. in Computer Science</strong> online scholar with a foundational background in <strong className="text-emerald-400 font-semibold">Diploma in Forestry (BTEB Regulation 2022)</strong>. Backed by 22+ professional industry certifications from Microsoft, Google, IBM, and Meta.
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 pt-1 text-xs font-mono text-slate-300">
              ...
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900/80 border border-cyan-500/30 text-cyan-300">
                ...
                <span>B.S. CS Running (UoPeople, USA)</span>
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900/80 border border-emerald-500/30 text-emerald-300">
                <Building2 className="w-4 h-4 text-emerald-400" />
                <span>Diploma in Forestry (BTEB)</span>
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900/80 border border-amber-500/30 text-amber-300">
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 pt-4">
              <button
                ...
                className="px-6 py-3 rounded-xl font-mono text-xs font-bold bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-400 hover:to-purple-500 text-white transition-all shadow-[0_0_25px_rgba(255,42,133,0.4)] flex items-center gap-2 cursor-pointer hover:scale-[1.02]"
              >
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Bachelor of Science in Computer Science (BCS) Card */}
          <div
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <span className="px-2.5 py-0.5 rounded text-[11px] font-mono font-bold bg-cyan-500/20 border border-cyan-400/30 text-cyan-300">
                  ONLINE UNDERGRADUATE DEGREE
                </span>
                <h3 className="text-xl font-bold text-white mt-2">Bachelor of Science in Computer Science (BCS)</h3>
                <p className="text-xs font-mono text-cyan-400 font-semibold mt-1">
                  University of the People (UoPeople), USA
                </p>
              </div>
              <span className="text-xs font-mono text-cyan-300 bg-cyan-950/60 px-2.5 py-1 rounded-lg border border-cyan-500/30 font-semibold flex items-center gap-1">
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-sans">
              Tuition-free, U.S.-accredited online degree program covering Data Structures, Algorithms, Software Engineering, Operating Systems, Database Management, Computer Architecture, Web Programming, and Artificial Intelligence.
            </p>

            <div className="pt-2 border-t border-slate-800 text-[11px] font-mono text-slate-400 space-y-1">
              <p>• Program Focus: Algorithms, Discrete Mathematics, Full-Stack Web Development, OOP in Java/C++, Software Design Patterns, AI & Machine Learning.</p>
              <p>• Accreditation: Accredited by DEAC, recognized by the U.S. Department of Education.</p>
            </div>

            <div className="pt-3 border-t border-slate-800/80 flex items-center">
              <a
                href="https://www.uopeople.edu/programs/online-bachelors/computer-science/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-medium transition-all hover:scale-[1.02] cursor-pointer"
              >
                <ExternalLink className="w-3.5 h-3.5 text-cyan-400" />
                <span>View Program Details (uopeople.edu)</span>
              </a>
            </div>
          </div>
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className="text-xs font-mono text-emerald-400 bg-emerald-950/60 px-2.5 py-1 rounded-lg border border-emerald-500/30 font-semibold flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                <span>Completed (4 Years)</span>
              </span>
            </div>

            <p className="text-xs text-slate-300 leading-relaxed font-sans">
              Completed 161 Credit Hours (4-Year B.Sc. Program) under the Outcome Based Education (OBE) Curriculum. Specialized in Power Systems, High-Voltage Dielectrics, VLSI Design, DSP, Microprocessors, Control Systems, and Renewable Solar/Wind Energy.
            </p>
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center">
              <a
                href="/syllabi/BSc_EEE_Syllabus_SUB.pdf"
                download="BSc_EEE_Syllabus_Southern_University.pdf"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-pink-500/10 hover:bg-pink-500/20 text-pink-300 border border-pink-500/30 text-xs font-mono font-medium transition-all hover:scale-[1.02] cursor-pointer"
              >
                <Download className="w-3.5 h-3.5 text-pink-400" />
                <span>Download B.Sc. EEE Syllabus (PDF)</span>
              </a>
            </div>
          </div>

          {/* Diploma in Forestry Card */}
          <div
            className={`p-6 rounded-3xl border backdrop-blur-xl space-y-4 ${
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <span className="px-2.5 py-0.5 rounded text-[11px] font-mono font-bold bg-emerald-500/20 border border-emerald-400/30 text-emerald-300">
                  DIPLOMA IN FORESTRY
                </span>
                <h3 className="text-xl font-bold text-white mt-2">Diploma in Forestry (Regulation 2022)</h3>
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-sans">
              Comprehensive 8-semester curriculum under BTEB Regulation 2022. Built rigorous expertise in Forest Ecology, Silviculture, Soil Conservation & Watershed Management, GIS & Remote Sensing, Mangrove & Coastal Afforestation, Wildlife Management, and Environmental Impact Assessment.
            </p>

            <div className="pt-2 border-t border-slate-800 text-[11px] font-mono text-slate-400 space-y-1">
              <p>• Core Syllabus: Forest Mensuration, GIS & Remote Sensing, Plantation Silviculture, Geology & Soil Science, Biodiversity Conservation.</p>
              <p>• Practical & Field Training: 8th Semester Field Attachment Training & Capstone Project Presentation.</p>
            </div>

            <div className="pt-3 border-t border-slate-800/80 flex items-center">
              <a
                href="/syllabi/Diploma_in_Forestry_Syllabus_BTEB_2022.pdf"
                download="Diploma_in_Forestry_Syllabus_BTEB_2022.pdf"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs font-mono font-medium transition-all hover:scale-[1.02] cursor-pointer"
              >
                <Download className="w-3.5 h-3.5 text-emerald-400" />
                <span>Download Forestry Syllabus (PDF)</span>
              </a>
            </div>
          </div>
        </div>
      ...
      <section className="p-8 rounded-3xl bg-slate-900/90 border border-purple-500/30 space-y-6 font-mono">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-xs">
          ...
          <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
            ...
            <p className="text-[10px] text-slate-400 font-sans">Google, MS, IBM, Meta, Coursera</p>
          </div>
